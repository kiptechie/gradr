  ...
  stepSix(payload) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        halt("The #oracle BUTTON does not have the specified CSS style");
      }
...
const challengeFour = {
  stepOne(payload) {
    return new Promise((resolve, reject) => {
      ...
      const declaresEndpoint = ast => {
        const [node] = astq.query(ast, `
          // VariableDeclarator [ /Identifier [@name == 'api']  &&/Literal [@value == 'https://randomapi.com/api/y1lfp11q?key=LEIX-GF3O-AG7I-6J84'] ]
        `);

        if(!node) {
          halt(`In "fetchAndDisplayUsers", declare an "api" variable and set its value as specified in the instructions`);
        }
      };

      const callsFetchWithAPI = ast => {
        const [node] = astq.query(ast, '// CallExpression [ /Identifier [@name == "fetch"] ]');
        const found = node && node.arguments && node.arguments[0] &&
        node.arguments[0].type == 'Identifier' &&
        node.arguments[0].name == 'api';

        if(!found) {
          halt(`In "fetchAndDisplayUsers", use the browser's fetch function to make a HTTP call to "api"`);
        }
      };

      const convertsDataToJSON = ast => {
        const nodes = astq.query(ast, '// MemberExpression [ /Identifier [@name == "json"] ]');
        const found = nodes.find(({property}) => property && property.name === 'json');
        if(!found) {
          halt(`Your "fetch" call should have a ".then" that converts the fetch response to JSON`);
        }
      };

      const handlesFetchedUser = ast => {
        let nodes = astq.query(ast, `
          // ArrowFunctionExpression /ObjectPattern /Property [ /Identifier [@name == 'results'] ]
        `);
        if(!nodes || nodes.length === 0) {
          halt('Make a ".then" with an inline arrow function that de-structures its parameter to get the "results" property. See the instructions');
        }

        nodes = astq.query(ast, `
          // ArrowFunctionExpression //VariableDeclarator [/ArrayPattern /Identifier [@name == 'user'] && /Identifier [@name == 'results']]
        `);
        if(!nodes || nodes.length === 0) {
          halt('Using array de-structuring get the first item from "results" and assign it to a "user" variable. See the instructions');
        }

        nodes = astq.query(ast, `
          // ArrowFunctionExpression //ExpressionStatement /CallExpression [
            /Identifier [@name == 'displayUsers'] &&
            /ArrayExpression /Identifier [@name == 'user']
          ]
        `);
        if(!nodes || nodes.length === 0) {
          halt('You need to call "displayUsers" with "user", but wrap it as an inline array');
        }
      };

      const ast = esprima.parseScript(script);
      declaresEndpoint(ast);
      callsFetchWithAPI(ast);
      convertsDataToJSON(ast);
      handlesFetchedUser(ast);

      setTimeout(() => {
        if(users.length < 3) {
          halt('Your code looks good but no user is being added as a result of your API call. Pls take another look');
        }
        resolve(payload);
      }, 3000);
    });
