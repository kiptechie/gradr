...
const challengeTwo = {
  stepOne(payload) {
    return new Promise((resolve, reject) => {
      ...
      let headingMarginBottom = computedPropertyStyle('h2', 'margin-bottom');
      if (headingMarginBottom.replace('px', '') < 65)
        reject(
  ...
  stepTwo(payload) {
    return new Promise((resolve, reject) => {
      ...
      const detailsHeight = computedPropertyStyle('.details', 'min-height');

      if (detailsHeight.replace('px', '') < 220) {
        reject(
          new Error(
    ...
    return new Promise((resolve, reject) => {
      ...
      const hasListener = (node, listenerFnName) => {
        if (nodeIsDisplayExtraUserInfoFn(node)) {
          ...
          return expressions.find(expr => findListener(expr, listenerFnName));
        }
        return false;
      };
